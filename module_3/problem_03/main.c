...
extern size_t current_count;

void loadContacts(PhoneBook* pb)
{
  int fd;
  if ((fd = open(FILE_NAME, O_RDONLY, 0)) == -1) {
    perror("ОШибка котрытия файла\n");
    return;
  }
  
  PhoneBook NewContact;
  while(read(fd, &NewContact, sizeof(PhoneBook)) > 0)
    addPerson(pb,&NewContact);
  close(fd);
}

void saveContacts(PhoneBook *pb)
{
  ...
  for (int i = 0; i < current_count; ++i)
    if (write(fd, &pb[i], sizeof(pb[i])) == -1) {
