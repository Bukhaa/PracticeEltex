...
#define FILENAME "out_11.txt"
#define read_ 0
#define write_ 1
#define COUNTPROC 2
#define SEM_NAME "semaphore"

void lock_processes(sem_t* sem){
    for(size_t k = 0; k < COUNTPROC; ++k){
	if(sem_wait(sem) == -1){
...
void unlock_processes(sem_t* sem){
    for(size_t k = 0; k < COUNTPROC; ++k){
        if(sem_post(sem) == -1){
           perror("error post_wait\n");
           exit(EXIT_FAILURE);
        }
    }           
}
...
int main(int argc, char* argv[]) {
    ...
    if ((sem = sem_open(SEM_NAME, O_CREAT | O_EXCL, 0660, COUNTPROC)) == SEM_FAILED){
